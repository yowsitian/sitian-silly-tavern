...
console.log(
    `Node version: ${process.version}. Running in ${process.env.NODE_ENV} environment.`
);
...
const server_port =
    cliArguments.port ??
    process.env.SILLY_TAVERN_PORT ??
    getConfigValue("port", DEFAULT_PORT);
const autorun =
    (cliArguments.autorun ?? getConfigValue("autorun", DEFAULT_AUTORUN)) &&
    !cliArguments.ssl;
const listen = cliArguments.listen ?? getConfigValue("listen", DEFAULT_LISTEN);
const enableCorsProxy =
    cliArguments.corsProxy ??
    getConfigValue("enableCorsProxy", DEFAULT_CORS_PROXY);
const enableWhitelist =
    cliArguments.whitelist ??
    getConfigValue("whitelistMode", DEFAULT_WHITELIST);
const dataRoot = cliArguments.dataRoot ?? getConfigValue("dataRoot", "./data");
const disableCsrf =
    cliArguments.disableCsrf ??
    getConfigValue("disableCsrfProtection", DEFAULT_CSRF_DISABLED);
const basicAuthMode =
    cliArguments.basicAuthMode ??
    getConfigValue("basicAuthMode", DEFAULT_BASIC_AUTH);
    ...
    app.use("/proxy/:url(*)", async (req, res) => {
        ...
        try {
            ...
            const response = await fetch(url, {
                method: req.method,
                headers: headers,
                body: bodyMethods.includes(req.method)
            ...
            res.status(500).send(
                "Error occurred while trying to proxy to: " + url + " " + error
            );
        }
    });
} else {
...
app.use(
    ...
    require("./src/endpoints/backends/text-completions").router
);

// KoboldAI
app.use(
    "/api/backends/kobold",
    require("./src/endpoints/backends/kobold").router
);

// OpenAI chat completions
app.use(
    "/api/backends/chat-completions",
    require("./src/endpoints/backends/chat-completions").router
);

// Scale (alt method)
app.use(
    "/api/backends/scale-alt",
    require("./src/endpoints/backends/scale-alt").router
);
